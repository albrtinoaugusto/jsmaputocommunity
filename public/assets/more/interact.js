...
function showChatHeader(){
    ...
    if (lesson.ownerAvatar.lenght > 20) {
        img.src = lesson.ownerAvatar;
    } else {
        img.src = "../../../_assets/as2/images/user-small.png";
    }
    img.alt = "Avatar";
    div3.appendChild(img);

    let div4 = document.createElement("div");
    div4.className = "chat-about";
    div3.appendChild(div4);

    let h6 = document.createElement("h6");
    h6.className = "m-b-0";
    h6.innerText = lesson.ownerName;
    div4.appendChild(h6);

    let span = document.createElement("span");
    span.innerText = lesson.dateTime.split(" - ")[1];
    div4.appendChild(span);

    headerChat.appendChild(div2);
}
...
function sendComment() {
    ...
    if (typeof(theLesson.comments) !== "undefined" && theLesson.comments.length > 0) {
        let counter = theLesson.comments.length + 1;
    ...
    if (typedComment.length > 0) {
        ...
        db.collection("lessons").doc(theLesson.id).update({comments: dataArray}).then(function(){
            ...
            if (typeof(avatar) === "undefined" || avatar === null) {
                avatarImg.src = "../../../_assets/as2/images/user-small.png";
            } else {
...
function getLessons(){
    db.collection("lessons").where('grade', '==', myData.grade).where("classId", "==", myData.classId).get().then(function (rs){
